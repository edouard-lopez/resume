...
// alert('DOM loaded');
  ...
  printFriendly();
  facetedSearch();
  getFilterPatternFromQueryString();
...
//     updateFilters($(this)); // operator OR
  updateView();
    setTimeout(function() { // run AT THE END (x+5ms) of the events
      ...
      setQueryStringOnAllLinks(buildQueryString()); // include URL, languages' tab, Anchor
...
//   var anchor = getFragmentFromHref();
  var ampersand = '';
  var queryString = '';
...
//     { ampersand = '&'; }
  ...
  {// a filter is defined by the facet search form
    ...
    {// Query string contains other arguments
      queryString = getQueryStringExcept('show')+'&show='+filters;
    } else 
    {
      queryString = 'show='+filters;
    }
  }
...
function getQueryStringExcept(arg) 
{// return the queryString less the 'arg' key/value
  var queryString = ''; // getQueryString();
  var ampersand = '';
  
  $.each($.url().param(), function(idx, val) {
    if (idx != arg)
    {
      queryString = queryString.concat(ampersand+idx+'='+val);
    }
    ampersand = '&';
  });
...
function setFilterPatternAsQueryString()
  { 
    setQueryString(buildQueryString()); 
  }
  
function getFilterPatternFromQueryString()
{
//   console.log("\tgetFilterPatternFromQueryString");
...
function setQueryStringOnAllLinks(qs)
{
  var qs = buildQueryString();
  var path = ["#menu-lang > ol#lang > li > a[href*=lang]", "#main>section>h2>a, #aside>section>h2>a"];
  setQueryString(qs);
  setQueryStringTo(path, qs);
}

function setQueryString(qs)
{ // http://stackoverflow.com/questions/1822598/getting-url-hash-location-and-using-it-in-jquery
//   location.search = qs; // return everything following the Hashtag (e.g.: #project|cog,web)  
}
function getQueryString()
{ return location.search; }

function setFragment(qs)
{ // http://stackoverflow.com/questions/1822598/getting-url-hash-location-and-using-it-in-jquery
  window.location.hash = qs; // return everything following the Hashtag (e.g.: #project|cog,web)  
}
function getFragment()
{ return window.location.hash; }

function setQueryStringTo(path, qs)
{// update all queryString of element matching 'path' selector
  $.each(path, function(key, itemPath) 
  {// for each 'path' selector 
    $(itemPath).each(function(key, val) 
    {// parse each item
      var hrefVal = $(this).url();
      var queryString = ''; // getQueryString();
      var ampersand = '';

      $.each(hrefVal.param(), function(idx, val) 
      ...
      queryString = queryString.concat(ampersand+'show='+getFilterPattern());
      
      console.log('Update from: '+hrefVal.attr('query')+' to: '+queryString);
