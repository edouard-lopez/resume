<?php
/**
 * Layout and semantic of the CV
 *
 * @category Front-End
 * @package  CV
 * @author   Édouard Lopez <dev+cv@edouard-lopez.com>
 * @license  http://creativecommons.org/licenses/by-nc-sa/3.0/legalcode CC-by-nc-sa-3.0
 * @link     http://edouard-lopez.com/cv
 */
...
/**
 * Shortcut to build <dd> element
 *
 * @param string  $str   content of the element
 * @param boolean $class HTML class for the element
...
<div class="overview">
    <dl id="qck-name">
        <dt class="h"><?= _cv("user-name") ?></dt>
        <dd><h1 id="name" class="id fn n">
                <span class="given-name"><?= $GLOBALS['me']->getFirstName() ?></span>
                <!--           <br/> -->
                <span class="family-name"><?= $GLOBALS['me']->getLastName() ?></span>
            </h1>
        </dd>
    </dl>
    ...
    <section id="summary" class="headline title">
        <div class="floatFix">
            <ul id="keywords">
                <li>
                    <strong><?= _cv("cv_keyword_1") // Développement Web ?></strong>,
                </li>
                <li><strong><?= _cv("cv_keyword_2") // Ergonomie ?></strong>
                    &amp; </li>
                <li>
                    <strong><?= _cv("cv_keyword_3") // Architecture de l'Information ?></strong>
                </li>
            </ul>
...
<section id="experiences" class="no-margin-top">
    <h2><?= _cv("cv.header.experiences"); ?><a href="#experiences"
                                               class="pilcrow">&para;</a></h2>
    <ol class="vcalendar">
        <?php
        $this->detailsListItem(
            array(
                ...
                'org'         => array(
                    'title'       => sprintf(_cv("cv.xp.title.ajlsm")), // role title = key idea/concept to remember
                    'org-summary' => sprintf(_cv("cv.org.name%s"), AJLSM), // company name
                    'location'    => sprintf(_cv("cv.org.place%s"), BDX) // company location
                ),
                'description' => array(
                    'content' => sprintf(
                        "<li>%s&thinsp;;</li>
                        ...
                        <li>%s&thinsp;;</li>
                        <li>%s.</li>",
                        sprintf(_cv("Développement d'applications %s/%s de gestion et de publication de documents %s"), J2EE, XSL_T, XML),
                        sprintf(_cv("Administration de serveurs %s et %s (%s avancés)"), DEBIAN, CENTOS, BASH),
                        sprintf(_cv("Gestion de projets&thinsp;: %s, %s et %s"), GIT, SVN, REDMINE),
                        sprintf(_cv("Capitalisation des savoir-faire (%s, %s)"), MEDIAWIKI, XMIND)
                    ),
                    'wrapper' => 'ul'
                    // Développeur J2EE/XSL (création de logiciels documentaires basés sur les technologies XML, Java, Tomcat, Cocoon, ...)
                    // Déploiement et administration de serveurs dédiés de tests et de production (CentOS, Debian) : Tomcat, Apache, PostgreSQL,
                    // Responsable des systèmes de gestion de versions (CVS, Subversion, Mercurial, GIT)
                ), // role description = longer description
                'period'      => array( // allow to compute approximative job duration: week(s) XOR month(s) XOR year(s)
                    'start' => '2011-10-03', // job start
                    // 'end' => date('Y-m-d'), // ... end
                    'end'   => '2013-03-29', // ... end
                ),
        ...
        $this->detailsListItem(
            array(
                ...
                'org'         => array(
                    ...
                    'org-summary' => sprintf(_cv("cv.org.name%s"), BDX2_URL),
...
<section id="education">
    <h2><?= _cv("cv.header.education"); ?><a href="#education"
                                             class="pilcrow">&para;</a></h2>
    <ol class="vcalendar">
        <?php
        $this->detailsListItem(
            array(
                ...
                'org'         => array(
                    ...
                    'org-summary' => sprintf(_cv("cv.org.name.at%s"), UPMF_XPAND_URL),
                    'location'    => sprintf(_cv("cv.org.place%s"), GRENOBLE)
                ),
                'description' => null,
                'period'      => array(
        ...
        $this->detailsListItem(
            array(
                ...
                'org'         => array(
                    ...
                    'org-summary' => sprintf(_cv("cv.org.name.at%s"), BDX2_XPAND_URL),
                    'location'    => sprintf(_cv("cv.org.place%s"), BDX)
                ),
                'description' => null,
                'period'      => array(
...
<h2><?= _cv("cv.header.projects"); ?><a href="#projects"
                                        class="pilcrow">&para;</a>
    <span class='tip left b-05e b-s-5p'><?= sprintf(_cv("detailled_portfolio%s"), "<a href='/portfolio'>portfolio</a>"); ?></span>
</h2>
<ol class="vcalendar">
<?php
$this->detailsListItem(
    array(
        'section'     => 'project',
        'org'         => array(
            'title'       => sprintf(_cv("cv.proj.probe")),
            'org-summary' => sprintf(_cv("cv.org.collaboration:personal")),
            'location'    => null
        ),
        'description' => sprintf(_cv("cv.proj.desc.probe%s%s%s"), D3js, CI, PROBE_METEO_COM),
        'period'      => array(
            'start' => '2012-10-31',
            'end'   => date('Y-m-d')
        ),
...
$this->detailsListItem(
    array(
        ...
        'description' => sprintf(_cv("cv.proj.desc01%s%s"), CFDICT_URL, CC_BY_NC_SA),
        'period'      => array(
            'start' => '2010-07-01',
            'end'   => date('Y-m-d')
        ),
...
$this->detailsListItem(
    array(
        ...
        'description' => sprintf(_cv("cv.proj.desc11%s%s"), sprintf("<a href='/portfolio/2008-Fifteen-Puzzle/'>%s</a>", _cv("15-puzzle")), IA_XPAND_URL),
        'period'      => array(
            'start' => '2008-11-30',
            'end'   => '2008-12-31',
        ),
...
<section id="aside">
    <section id="skills" class='section flt-right no-margin-top'>
        <h2><?= _cv("Skills"); ?><a href="#skills" class="pilcrow">&para;</a>
        </h2>
        <?php
        $scico = new dl();
        $scico->setId('scico');
        ...
        print $scico->buildOutput(
            array(
                ...
                array(
                    ...
                    new skill(_cv("Statistics"), array('cog', 'ux', 'web'), array('2005', '2009'), -1, true)
                ),
                new dt(_cv("Back-end")),
                array(
                    ...
                    new skill(COCOON, array('dev'), array('1900', '1901'), -1),
                    new skill("Delphi", array('dev'), array('1900', '1901'), -1),
                    new skill("Perl", array('cog', 'dev', 'ux'), array('1900', '1901'), -1, true)
                ),
                new dt(_cv("Front-end")),
                array(
                    new skill(LESS, array('dev', 'web', 'ux'), array('1999', $tb->now('Y-m')), -1),
                    new skill(SCSS, array('dev', 'web', 'ux'), array('1999', $tb->now('Y-m')), -1),
                    new skill(D3js, array('dev', 'web', 'ux'), array('1999', $tb->now('Y-m')), -1),
                    new skill(HANDLEBARS, array('dev', 'web', 'ux'), array('1999', $tb->now('Y-m')), -1),
                    new skill(GRUNT, array('dev', 'web', 'ux'), array('1999', $tb->now('Y-m')), -1),
                ...
                array(
                    ...
                    new skill(XMIND, array('dev', 'cog'), array('1900', '1901'), -1, true),
                ),
                ...
                array(
                    ...
                    new skill(XPATH, array('dev', 'web'), array('1900', '1901'), -1),
                    ...
                    new skill(XUL, array('dev', 'web', 'ux'), array('1900', '1901'), -1, true)
                ),
                new dt(_cv("Database")),
                array(
                    new skill(SQL, array('dev', 'web'), array('1900', '1901'), -1),
                    new skill("PostgreSQL", array('dev', 'web'), array('1900', '1901'), -1),
                    new skill("MySQL (Workbench)", array('dev', 'web'), array('1900', '1901'), -1),
                    new skill("phpMyAdmin", array('dev', 'web'), array('1900', '1901'), -1),
//                    new skill("<del>Access</del>", array('dev', 'web'), array('1900', '1901'), -1, true)
                ),
                new dt(_cv("Server")),
                array(
                    new skill(TOMCAT, array('i18n', 'web'), array('1900', '1901'), -1),
                    new skill(HTTPD, array('i18n', 'web'), array('1900', '1901'), -1),
                    new skill(SFTP, array('web'), array('1900', '1901'), -1, true)
                ),
                new dt(_cv(OS)),
                array(
                    new skill(DEBIAN, array('dev'), array('1900', '1901'), -1),
//               new skill( UBUNTU, array('dev'), array('1900', '1901'), -1 ),
                    new skill(CENTOS, array('dev'), array('1900', '1901'), -1),
                    new skill(GENTOO, array('dev'), array('1900', '1901'), -1),
                    new skill("VirtualBox", array('dev'), array('1900', '1901'), -1),
                    new skill(sprintf("Windows"), array('dev'), array('1900', '1901'), -1, true)
                ),
                ...
                array(
                    ...
                    new skill("R", array('cog', 'dev'), array('1900', '1901'), -1, true)
                ),
                new dt(_cv("Design")),
                array(
                    new skill("The Gimp", array('gfx'), array('1900', '1901'), -1),
                    new skill("Inkscape", array('gfx'), array('1900', '1901'), -1, true),
//               new skill( "Photoshop", array('gfx'), array('1900', '1901'), -1 ),
                ...
                array(
                    ...
                    new skill(C2I, array('off'), array('1900', '1901'), -1, true),
                )
            )
        );
        ?>
    ...
    <section id="Languages" class='i18n'>
        <h2><?= _cv("Languages"); ?><a href="#Languages"
                                       class="pilcrow">&para;</a></h2>
        <?php
        $languages = new dl();
...
{
    ?>
    <section id='<?= $id; ?>' class='section flt-right'>
        <h2><?= _cv("Spare Time"); ?><a href="#<?= $id; ?>"
                                        class="pilcrow">&para;</a></h2>
        <?php
        $sport = new dl();
        ...
        print $travel->buildOutput(
            array(
                new dt(_cv("Travels")),
                array(
                    _dd(sprintf("%s (%s)", AMSTERDAM, _cv("'13"))),
                    _dd(sprintf("%s (%s)", LONDON, _cv("'12"))),
                    _dd(sprintf("%s (%s, %s, %s)", _cv("Taiwan"), _cv("'11"), _cv("'10"), _cv("'04"))),
...
<h2 class="h"><?= _("Additional Information"); ?> </h2>
<footer id="additional-info">
    <div id="additional-info-wrapper">
        <h3 class="h"><?= _("Personal Information"); ?> </h3>
        <dl id="personal-info">
            <!--       <dt class="h"><?=_cv("user_age"); ?></dt> -->
            <!--         <dd><?=$age.' '._cv("user_years_old"); ?>, </dd> -->
            <dt><?= _cv("user_nationality"); ?></dt>
            <dd><?= _cv("user_french"); ?>,</dd>
            <dt><?= _cv("user_transport"); ?></dt>
            <dd><?= _cv("user_driving_license"); ?>,</dd>
            <dt><?= _cv("Sex"); ?></dt>
            <dd><?= _cv("Male"); // SEX_MALE ?>.</dd>
            <dt><?= _cv("Status"); ?></dt>
            <dd><?= _cv("Single"); ?>.</dd>
        </dl>

        <h3 class="h"><?= _("Contact Information"); ?> </h3>
        <dl id="contact-me">
            <dt><?= _cv("user_address"); ?></dt>
            <dd class="adr">
                <address>
                    <p class="locality"> Aquitaine, France </p>
                </address>
            </dd>
            <dt id="email"><?= _cv("user_email"); ?></dt>
            <dd class="email">cv+me@edouard-lopez.com</dd>
            <dt><?= _cv("user_phone"); ?></dt>
            <!--         <dd class="tel cell">+33.6.10.41.53.65</dd> -->
            <dd class="tel cell">+33.(0)7.60.06.97.30</dd>
            <dt><?= _cv("Website"); ?></dt>
            <dd><span class="url fn"><a href="http://edouard-lopez.com/">edouard-lopez.com/</a></span>
            </dd>
        </dl>
        <div id="bottomer" />
    </div>
...
<footer id='legend'>
    <h3><?= _("Skills' Legend"); ?></h3>
    <dl>
        <dt><?= _("Academic skills"); ?> </dt>
        <dd><?= sprintf(_cv("use a scale from 1 to 3, respectively representing %s"), sprintf("%s, %s and %s degrees' levels.", _cv('Bachelor'), _cv('Master'), PhD)); ?></dd>
        <dt><?= _("Programming skills"); ?> </dt>
        <dd><?= sprintf(_cv("use a scale from 1 to 3, respectively representing %s"), sprintf("%s, %s and %s degrees' levels.", _cv('Bachelor'), _cv('Master'), PhD)); ?></dd>
    </dl>
</footer>
